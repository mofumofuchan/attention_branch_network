...
from imagenet_train import train, ResNet50WithNewAttentionBranch
from imagenet_eval import test
...
for name in customized_models.__dict__:
    if name.islower() and \
       ...
       callable(customized_models.__dict__[name]):
...
parser.add_argument('--resume_feature_extractor', type=str,
                    help='resume weights of feature extractor')
...
def main():
    ...
    if args.resume_feature_extractor:
        print('==> Resuming weights of feature extractor..')

        checkpoint = torch.load(args.resume_feature_extractor)
        model.load_state_dict(checkpoint)

    # wrap original model
    model = ResNet50WithNewAttentionBranch(model)
...
def save_checkpoint(state, is_best,
                    checkpoint='checkpoint', filename='checkpoint.pth.tar'):
